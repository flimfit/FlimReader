...
typedef struct {          /* structure for GVD module data  */
  ...
  short  sreserve;      //  reserve for future changes
  double dreserve[2];   //  reserve for future changes
  }GVDdata;
...
typedef struct{           // structure for FCS fitting parameters
  ...
  double      tau_trip;   //  triplet decay time [�s]
  double      y_1;        //  fraction of 1st component [%]
  double      tau_1;      //  diffusion time (1st comp.) [�s]
  double      tau_2;      //  diffusion time (2nd comp.) [�s]
...
typedef struct{        // structure for FCS fitting parameters
  ...
  short       fix;      //  bitwise value shows which fields of FITPar are fixed
                        //       during fitting
  double      st_time;  // start time of fitting [�s]
...
typedef struct{        // structure for PLIM measurement support using DDG-210 module
  ...
  float              ldel_time;     // Laser delay time in �s
  float              lwidth_time;   // Laser width time in �s
  float              rdel_time;     // Routing delay time in �s
  float              rwidth_time;   // Routing width time in �s
  BHPanelAttr        pnl_attr; 
                                      //   - now all rest channels have the same settings
                                      //   but in future - can be different
  float              del_time[6];     // delay time in �s for all 6 channels 
  float              width_time[6];   // width time in �s for all 6 channels
...
typedef struct{             //  structure for Wide-field Image mode parameters
  ...
  short  tac_gain_Tmod;     //  TAC gain for T module,     1 .. 15
  short  tac_gain_XYmod;    //  TAC gain for XY modules,   1 .. 15
  float  tac_range_Tmod;    //  TAC range for T module,    50 .. 5000 ns
  float  tac_range_XYmod;   //  TAC range for XY modules,  50 .. 5000 ns
...
typedef struct{         // structure for extension of 3D trace parameters
  ...
  char   reserve[255];     // keep fixed size of the structure = 512B
  }TrParExt;
...
/*
 ...
 FCS histogram data contain results of FCS (auto or cross ) calculation for photons 
    ...
    FCS calculation uses logaritmic binning therefore both time ( x ) and data ( y ) 
      values are required.
    At first FCSInfo.fcs_points values of FCS time in [�s] ( x scale ) are  given 
